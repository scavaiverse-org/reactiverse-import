...
import HomeMediaField from "@/components/tenant-admin/HomeMediaField";
import HomePublishReadiness from "@/components/tenant-admin/HomePublishReadiness";
...
const emptyUploadStatuses = {
  logoUrl: { state: "idle" },
  heroMediaUrl: { state: "idle" },
  cardMediaUrl: { state: "idle" },
};
const hasMedia = (url) => typeof url === "string" && url.trim().length > 0;
...
export default function TenantHomeEditor() {
  const { tenant } = useActiveTenant();
  const queryClient = useQueryClient();
  const [draft, setDraft] = useState(defaultDraft(tenant));
  const [uploadStatuses, setUploadStatuses] = useState(emptyUploadStatuses);
  const [saveStatus, setSaveStatus] = useState("Idle / no changes saved yet");
  ...
  const persistedDraft = useMemo(() => draftFromConfig(tenant, draftRecord || publishedRecord), [tenant, draftRecord, publishedRecord]);
  const hasUnsavedChanges = useMemo(() => JSON.stringify(draft) !== JSON.stringify(persistedDraft), [draft, persistedDraft]);
  const isUploading = Object.values(uploadStatuses).some((status) => status.state === "uploading");
  const readinessChecks = useMemo(() => [
    { label: "Title present", ok: !!draft.heroTitle?.trim(), required: true },
    { label: "Description present", ok: !!draft.heroDescription?.trim(), required: true },
    { label: "Hero media present or fallback available", ok: hasMedia(draft.heroMediaUrl) || hasMedia(tenant?.theme_config?.background_image_url), required: false },
    { label: "Cinematic card media present or fallback available", ok: hasMedia(draft.cardMediaUrl) || hasMedia(draft.heroMediaUrl) || hasMedia(tenant?.theme_config?.background_image_url), required: false },
    { label: "Tenant ownership fields present", ok: !!tenant?.id && !!tenant?.slug, required: true },
    { label: hasUnsavedChanges ? "Draft has unsaved changes" : "Draft is saved", ok: !hasUnsavedChanges, required: false },
  ], [draft, tenant, hasUnsavedChanges]);
  const canPublish = readinessChecks.filter((item) => item.required).every((item) => item.ok);

  const markMediaStatus = (state) => {
    setUploadStatuses((prev) => Object.fromEntries(Object.entries(prev).map(([key, value]) => [key, { ...value, state }])));
  };
  const setFieldStatus = (field, status) => setUploadStatuses((prev) => ({ ...prev, [field]: { ...(prev[field] || {}), ...status } }));
  const update = (field, value) => {
    setDraft((prev) => ({ ...prev, [field]: value }));
    if (uploadStatuses[field]) setFieldStatus(field, { state: value ? "complete" : "idle", mediaType: detectMediaType(value), error: null });
    setSaveStatus("Unsaved changes");
  };
  const uploadMedia = async (field, typeField, file) => {
    if (!file) {
      setFieldStatus(field, { state: "idle", error: null });
      return;
    }
    ...
    try {
      ...
      setFieldStatus(field, { state: "failed", fileName: file.name, mediaType: file.type?.startsWith("video/") ? "video" : "image", error: error.message || "Upload failed" });
    }
  };

  const saveMutation = useMutation({
    mutationFn: async (action) => {
      const draftPayload = payloadFromDraft(tenant, draft, "draft");
      const savedDraft = draftRecord?.id ? await base44.entities.MuseumPageConfig.update(draftRecord.id, draftPayload) : await base44.entities.MuseumPageConfig.create(draftPayload);
      if (action === "published") {
        const publishedPayload = payloadFromDraft(tenant, draft, "published");
        const savedPublished = publishedRecord?.id ? await base44.entities.MuseumPageConfig.update(publishedRecord.id, publishedPayload) : await base44.entities.MuseumPageConfig.create(publishedPayload);
        return { action, savedDraft, savedPublished };
      }
      return { action, savedDraft };
    },
    onSuccess: async (_data, action) => {
      await queryClient.invalidateQueries({ queryKey: ["tenant-home-page-configs", tenant?.id] });
      await queryClient.invalidateQueries({ queryKey: ["tenant-public-page-config", tenant?.id, pageKey] });
      await queryClient.invalidateQueries({ queryKey: ["tenant-public-media", tenant?.id] });
      setSaveStatus(action === "published" ? "Published live" : "Saved to draft");
      markMediaStatus(action === "published" ? "published" : "saved");
    },
  });
    ...
    <main className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" disabled={saveMutation.isPending || isUploading} onClick={() => saveMutation.mutate("draft")}><Save className="h-4 w-4" /> Save Draft</Button>
          <Button disabled={saveMutation.isPending || isUploading || !canPublish} onClick={() => saveMutation.mutate("published")}><Eye className="h-4 w-4" /> Publish</Button>
        </div>
      </div>
      ...
      <HomePublishReadiness checks={readinessChecks} statusText={saveMutation.isPending ? "Saving..." : saveStatus} />
      ...
      <section className="grid gap-5 rounded-3xl border border-white/10 bg-white/[0.035] p-5 lg:grid-cols-2">
        ...
        <HomeMediaField label="Museum logo/avatar media" value={draft.logoUrl} mediaType={detectMediaType(draft.logoUrl)} status={uploadStatuses.logoUrl} onChange={(value) => update("logoUrl", value)} onUpload={(file) => uploadMedia("logoUrl", null, file)} disabled={saveMutation.isPending} />
        ...
        <HomeMediaField label="Hero background media" value={draft.heroMediaUrl} mediaType={draft.heroMediaType} status={uploadStatuses.heroMediaUrl} onChange={(value) => { update("heroMediaUrl", value); update("heroMediaType", detectMediaType(value)); }} onUpload={(file) => uploadMedia("heroMediaUrl", "heroMediaType", file)} disabled={saveMutation.isPending} />
        <HomeMediaField label="Right-side cinematic card media" value={draft.cardMediaUrl} mediaType={draft.cardMediaType} status={uploadStatuses.cardMediaUrl} onChange={(value) => { update("cardMediaUrl", value); update("cardMediaType", detectMediaType(value)); }} onUpload={(file) => uploadMedia("cardMediaUrl", "cardMediaType", file)} disabled={saveMutation.isPending} />
