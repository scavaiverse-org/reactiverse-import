        ...
        {tab === "advanced" && (
          <>
            ...
            <div className="space-y-3">
              ...
              {(room.hotspots || []).map((hotspot, index) => <div key={hotspot.id || index} className="grid gap-2 rounded-xl border border-white/10 bg-background/40 p-3 md:grid-cols-6"><Input className="md:col-span-2" value={hotspot.title || ""} placeholder="Title" onChange={(e) => updateList("hotspots", index, { ...hotspot, title: e.target.value, label: e.target.value })} /><Input type="number" value={hotspot.x || 0} placeholder="X %" onChange={(e) => updateList("hotspots", index, { ...hotspot, x: Number(e.target.value) })} /><Input type="number" value={hotspot.y || 0} placeholder="Y %" onChange={(e) => updateList("hotspots", index, { ...hotspot, y: Number(e.target.value) })} /><Input value={hotspot.cta_label || ""} placeholder="CTA" onChange={(e) => updateList("hotspots", index, { ...hotspot, cta_label: e.target.value })} /><Button variant="outline" onClick={() => removeList("hotspots", index)}>Remove</Button><Textarea className="md:col-span-6" rows={2} value={hotspot.description || ""} placeholder="Description" onChange={(e) => updateList("hotspots", index, { ...hotspot, description: e.target.value })} /></div>)}
            </div>
