...
export default function PerformanceStageEditor({ room, onChange }) {
  ...
  return (
    <section className="grid gap-4 rounded-2xl border border-primary/15 bg-primary/5 p-4 md:grid-cols-2">
      ...
      <label className="space-y-2 md:col-span-2"><Label>Script / lyrics</Label><Textarea rows={3} value={config.script_text || config.lyrics_text || ""} onChange={(e) => setConfig({ script_text: e.target.value, lyrics_text: e.target.value })} /></label>
