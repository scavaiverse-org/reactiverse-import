...
function getArtifactContextNote(room) {
  const artifactNames = (room.artifact_config?.artifacts || []).map((artifact) => artifact.title).filter(Boolean);
  const hotspotNames = (room.hotspots || []).map((hotspot) => hotspot.title || hotspot.label).filter(Boolean);
  const names = [...artifactNames, ...hotspotNames];
  if (!names.length) return "";
  return ` I can also speak to what's on display here: ${names.join(", ")}.`;
}
...
export default function AIConversationRoom({ room, context = {} }) {
  ...
  const artifactNote = config.artifact_context_enabled !== false ? getArtifactContextNote(room) : "";
  const send = (text = input) => { if (!text.trim()) return; context.track?.("walkthrough_ai_question_started", { question: text }); setMessages((prev) => [...prev, { role: "user", content: text }, { role: "assistant", content: `This room invites close looking, context, and reflection. Use the artifact notes and narration to guide your interpretation.${artifactNote}` }]); setInput(""); };
