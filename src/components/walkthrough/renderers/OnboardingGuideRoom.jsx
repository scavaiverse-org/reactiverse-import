...
export default function OnboardingGuideRoom({ room, onNext, onChoice }) {
    ...
    <div className="flex min-h-[70vh] items-center justify-center px-4 py-20">
      <div className="relative w-full max-w-2xl rounded-[2rem] border border-white/15 bg-background/75 p-8 text-center shadow-2xl backdrop-blur-xl">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:justify-center">
          {(config.choices || []).map((choice, index) => <Button key={choice.id || choice.label || `choice-${index}`} variant="outline" onClick={() => onChoice(choice)}>{choice.label}</Button>) }
